...
load(file.path("rdata", "report3_fullheading.Rdata"))
```

# `r report3_fullheading[1]`\

```{r list of pngs, echo = FALSE}
report3_pngs = dir("plots")[grep("report3",dir("plots"))]
```

```{r insert plots, echo = F, results = 'asis'}
for(i in seq_along(report3_pngs)){
    cat('![]')
    cat(paste("(plots/",report3_pngs[i],")",sep=""))
    cat('{ width=100%}\n')
}
```
\newpage
