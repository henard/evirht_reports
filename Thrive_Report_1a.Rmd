...
load(file.path("rdata", "report1a_fullheading.Rdata"))
```


# `r report1a_fullheading[1]`\

```{r list of pngs, echo = FALSE}
report1a_pngs = dir("plots")[grep("report1a",dir("plots"))]
```

```{r insert plots, echo = F, results = 'asis'}
for(i in seq_along(report1a_pngs)){
    cat('![]')
    cat(paste("(plots/",report1a_pngs[i],")",sep=""))
    cat('{height=14cm}\\newline')
}
```
