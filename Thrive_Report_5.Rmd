...
load(file.path("rdata", "report5_fullheading.Rdata"))
```

# `r report5_fullheading[1]`\


```{r list of pngs, echo = FALSE}
report5_pngs = dir("plots")[grep("report5",dir("plots"))]
```

```{r insert plots, echo = F, results = 'asis'}
for(i in seq_along(report5_pngs)){
    cat('![]')
    cat(paste("(plots/",report5_pngs[i],")",sep=""))
    cat('{width=100%}\n')
}
```
