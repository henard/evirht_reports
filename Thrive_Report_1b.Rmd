...
load(file.path("rdata", "report1b_fullheading.Rdata"))
```


# `r report1b_fullheading[1]`\

```{r list of pngs, echo = FALSE}
report1b_pngs = dir("plots")[grep("report1b",dir("plots"))]
```

```{r insert plots, echo = F, results = 'asis'}
for(i in seq_along(report1b_pngs)){
    cat('![]')
    cat(paste("(plots/",report1b_pngs[i],")",sep=""))
    cat('{width=100%}\\newline')
}
```
